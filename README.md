# Guías de Oportunidades y Carreras Tecnológicas

Recursos gratuitos y prácticos en español para ayudar a las personas a avanzar desde el aprendizaje hasta una oportunidad real.

Este proyecto reúne orientación sobre:

- Empleo
- Prácticas profesionales
- Pasantías
- Aprendizajes laborales
- Becas y fellowships
- Programas de mentoría
- Programas para recién graduados
- Programas de reintegración laboral
- Educación tecnológica gratuita
- Guías de carreras tecnológicas
- Preparación de portafolios y GitHub
- Currículum y entrevistas
- Accesibilidad y adaptaciones
- Prevención de estafas laborales
- Inglés gratuito para tecnología
- Orientación profesional centrada en la persona

---

## Misión

Muchas personas tienen la capacidad de generar un cambio positivo en el mundo, pero no siempre cuentan con dinero, orientación, adaptaciones, contactos o acceso a información confiable.

Este proyecto existe para ayudar al menos a una persona a utilizar la tecnología para mejorar el mundo y motivarla a ayudar a otra persona.

> **Ayuda a una persona. Comparte una oportunidad verificada. Enseña una habilidad útil. Deja el camino más claro de cómo lo encontraste.**

---

## ¿A quién está dirigido este proyecto?

Este proyecto puede ser útil para:

- Estudiantes
- Personas que buscan su primer empleo
- Profesionales que desean cambiar de carrera
- Personas desempleadas
- Adultos mayores
- Personas con discapacidad
- Personas neurodivergentes
- Personas con interrupciones laborales
- Personas con recursos económicos limitados
- Personas que viven fuera de los principales centros tecnológicos
- Profesionales que desean trabajar de forma remota
- Personas que necesitan mejorar su inglés
- Mentores, docentes y organizaciones comunitarias

No es necesario tener un título universitario, experiencia previa o inglés perfecto para comenzar.

---

## Comienza aquí

El proyecto se desarrollará en cuatro áreas principales.

### 1. Oportunidades tecnológicas globales

Recursos para encontrar y evaluar:

- Empleo
- Pasantías
- Prácticas profesionales
- Aprendizajes laborales
- Fellowships
- Becas
- Mentorías
- Traineeships
- Programas para recién graduados
- Programas de retorno al trabajo
- Oportunidades remotas
- Oportunidades accesibles e inclusivas

Toda oportunidad deberá verificarse, cuando sea posible, mediante el sitio oficial del empleador, universidad, gobierno, organización sin fines de lucro o institución responsable.

### 2. Educación gratuita

Se incluirán recursos como:

- Cursos completamente gratuitos
- Cursos gratuitos para auditar
- Certificados gratuitos cuando estén disponibles
- Certificados de pago opcional
- Laboratorios prácticos
- Herramientas de código abierto
- Versiones gratuitas de software
- Documentación oficial
- Proyectos de portafolio
- Orientación para GitHub
- Preparación para currículum e entrevistas
- Uso responsable de inteligencia artificial

### 3. [Guías de carreras tecnológicas](03-guias-de-carrera/README.md)

La colección incluirá orientación sobre carreras relacionadas con:

- Inteligencia artificial
- Ciberseguridad
- Computación en la nube
- Datos
- Desarrollo de software
- Gobierno, riesgo y cumplimiento
- Privacidad
- Robótica
- Internet de las cosas
- Infraestructura tecnológica
- Tecnología cuántica
- Energía limpia
- Accesibilidad
- Tecnología de asistencia
- Tecnología centrada en la persona

Cada guía podrá incluir:

- Descripción de la carrera
- Responsabilidades comunes
- Puestos iniciales y avanzados
- Habilidades técnicas
- Habilidades humanas y de comunicación
- Rutas con y sin título universitario
- Recursos gratuitos
- Herramientas prácticas
- Proyectos de portafolio
- Evidencia para GitHub
- Certificaciones opcionales
- Preparación para entrevistas
- Accesibilidad y adaptaciones
- Consideraciones éticas y legales
- Planes de aprendizaje de 30, 60 y 90 días

### 4. Inglés gratuito para tecnología

Gran parte de la documentación técnica, las certificaciones, las herramientas y las oportunidades internacionales utiliza el inglés.

El proyecto incluirá recursos gratuitos y accesibles globalmente para desarrollar:

- Inglés básico
- Comprensión auditiva
- Lectura técnica
- Vocabulario tecnológico
- Pronunciación
- Escritura profesional
- Correos laborales
- Currículum en inglés
- Perfil de LinkedIn
- Entrevistas
- Presentación de proyectos
- Participación en reuniones
- Documentación de GitHub

El inglés no determina la inteligencia ni el potencial de una persona. El objetivo es desarrollar suficiente confianza para aprender, buscar información, hacer preguntas y explicar el propio trabajo.

---

## Plataformas gratuitas de inglés

Las siguientes plataformas ofrecen materiales gratuitos en línea. Algunas funciones, certificados o servicios adicionales pueden tener costo.

- [British Council LearnEnglish](https://learnenglish.britishcouncil.org/)  
  Lecciones de lectura, escritura, comprensión auditiva, pronunciación, gramática y vocabulario.

- [USA Learns](https://www.usalearns.org/)  
  Cursos estructurados de inglés para niveles iniciales e intermedios.

- [VOA Learning English](https://learningenglish.voanews.com/)  
  Noticias, videos, audios y lecciones preparados para estudiantes de inglés.

- [BBC Learning English](https://www.bbc.co.uk/learningenglish/)  
  Recursos de vocabulario, gramática, pronunciación, inglés cotidiano e inglés profesional.

- [Cambridge English](https://www.cambridgeenglish.org/learning-english/activities-for-learners/)  
  Actividades gratuitas organizadas por nivel y habilidad.

- [Duolingo](https://www.duolingo.com/)  
  Práctica diaria de vocabulario y construcción de oraciones mediante una modalidad gratuita.

Los usuarios deben verificar la disponibilidad regional, las condiciones de registro y cualquier costo relacionado con certificados.

---

## Principios del proyecto

### Ayudar mientras avanzamos

El éxito de otra persona no reduce el nuestro.

Invitamos a quienes se beneficien del proyecto a ayudar a otra persona mediante acciones como:

- Compartir una oportunidad verificada
- Explicar un requisito de solicitud
- Revisar un proyecto de principiante
- Traducir un recurso
- Mejorar la accesibilidad
- Reportar un enlace vencido
- Ofrecer ánimo después de un rechazo
- Ayudar a crear un primer currículum
- Orientar sobre un primer proyecto de GitHub

### La seguridad humana es primero

Ningún empleo, título, salario, certificación, entrevista o expectativa familiar es más importante que la seguridad, la dignidad y la vida de una persona.

Este proyecto no avergüenza a nadie por:

- Estar desempleado
- Tener una discapacidad
- Tener una edad avanzada
- No tener título universitario
- Tener interrupciones laborales
- Haber sido rechazado
- Tener dificultades económicas
- Necesitar más tiempo
- Cambiar de dirección
- Necesitar ayuda

Si la presión relacionada con el trabajo o los estudios está generando desesperanza o pensamientos de autolesión, la planificación profesional debe detenerse. La prioridad debe ser buscar apoyo inmediato de una persona de confianza, un profesional calificado, un servicio local de emergencia o un servicio de crisis.

### No causar daño

Nos comprometemos a:

- Orientar sin juzgar
- Motivar sin presionar
- Informar sin engañar
- Proteger la privacidad
- Evitar falsas promesas
- Respetar la discapacidad
- Respetar la edad
- Respetar la cultura y el idioma
- Respetar las circunstancias económicas
- Reconocer cuándo la orientación profesional debe detenerse
- Dar prioridad al apoyo humano cuando exista riesgo

### No al gatekeeping

Este proyecto no apoya:

- La ridiculización
- La discriminación
- El desaliento deliberado
- La explotación
- El plagio
- Las falsas promesas
- El fraude de contratación
- La humillación de principiantes
- La ocultación de oportunidades públicas detrás de pagos
- La idea de que el éxito de otra persona representa una amenaza

---

## Uso responsable de la inteligencia artificial

La inteligencia artificial puede ayudar a:

- Comparar carreras
- Comprender descripciones de empleo
- Corregir gramática
- Organizar planes de aprendizaje
- Practicar entrevistas
- Preparar preguntas de estudio
- Revisar documentación
- Generar ideas de proyectos
- Mejorar la claridad de un currículum

La inteligencia artificial no debe utilizarse para:

- Inventar experiencia laboral
- Crear certificaciones falsas
- Suplantar a otra persona
- Completar evaluaciones restringidas
- Plagiar proyectos
- Revelar información confidencial
- Presentar como cierta información no verificada
- Engañar a empleadores o instituciones educativas

Cada persona es responsable de verificar y aprobar toda información que presente.

---

## Compromiso con la accesibilidad

El proyecto busca apoyar a personas con:

- Discapacidades físicas
- Discapacidades sensoriales
- Discapacidades cognitivas
- Dificultades de aprendizaje
- Condiciones neurológicas
- Condiciones de salud mental
- Barreras lingüísticas
- Acceso limitado a equipos
- Acceso limitado a Internet
- Trayectorias educativas no tradicionales

Los documentos deberán utilizar, cuando sea posible:

- Encabezados claros
- Lenguaje sencillo
- Enlaces descriptivos
- Tablas accesibles
- Texto alternativo para imágenes
- Formato legible
- Archivos en varios formatos
- Lenguaje respetuoso e inclusivo
- Instrucciones comprensibles
- Explicaciones para principiantes

---

## Prevención de estafas laborales

Nunca se debe pagar por recibir una oferta de empleo legítima.

Señales de alerta frecuentes:

- Solicitudes de dinero antes de una entrevista
- Compra obligatoria de equipos mediante un proveedor desconocido
- Cheques enviados para comprar equipos
- Entrevistas realizadas únicamente por mensajería
- Direcciones de correo que no pertenecen a la empresa
- Ofertas sin entrevista
- Salarios poco realistas
- Solicitudes tempranas de información bancaria
- Presión para actuar inmediatamente
- Descripciones laborales copiadas o incompletas
- Solicitudes de contraseñas o códigos de autenticación

Antes de responder, se debe verificar:

- El sitio oficial de la empresa
- El dominio del correo
- El perfil del reclutador
- La existencia de la vacante
- La ubicación y elegibilidad
- El tipo de contrato
- La forma de pago
- La política de privacidad
- Los datos que realmente necesita el empleador

---

## Estándar editorial y de calidad

Cada documento deberá revisarse para verificar:

- Ortografía
- Gramática
- Puntuación
- Voz natural
- Lenguaje claro
- Exactitud factual
- Enlaces oficiales
- Accesibilidad
- Seguridad
- Privacidad
- Ética
- Coherencia de términos
- Calidad visual
- Vigencia de la información

Cuando sea posible, se utilizarán fuentes oficiales en lugar de resúmenes de terceros.

---

## Cómo contribuir

Son bienvenidas las contribuciones de:

- Estudiantes
- Solicitantes de empleo
- Profesionales
- Educadores
- Mentores
- Empleadores
- Universidades
- Organizaciones sin fines de lucro
- Especialistas en accesibilidad
- Comunidades de código abierto
- Exparticipantes de programas

Las contribuciones pueden incluir:

- Una oportunidad oficial
- Una corrección
- Un enlace actualizado
- Una traducción
- Una mejora de accesibilidad
- Un recurso gratuito
- Un proyecto práctico
- Una experiencia personal
- Una guía de entrevista
- Una ruta de aprendizaje
- Una recomendación de seguridad

Al compartir una oportunidad, incluya:

- Nombre del programa
- Organización
- Enlace oficial
- Tipo de oportunidad
- País o región
- Modalidad remota, híbrida o presencial
- Requisitos
- Compensación o estipendio
- Fechas de solicitud
- Área tecnológica
- Fecha de verificación

No incluya información confidencial, datos personales sensibles ni contenido protegido sin autorización.

---

## Aviso importante

Este proyecto se proporciona con fines educativos y de exploración profesional.

No constituye:

- Evaluación psicológica
- Consejo médico
- Consejo legal
- Consejo financiero
- Consejo migratorio
- Evaluación de rehabilitación
- Terapia
- Orientación profesional con licencia
- Garantía de empleo

Toda información importante debe verificarse mediante fuentes oficiales y profesionales calificados cuando corresponda.

---

## Estado del proyecto

La primera colección de carreras en español está disponible:

- [Guía para elegir una carrera tecnológica](03-guias-de-carrera/elige-tu-carrera-tecnologica.md)
- [Las 50 carreras tecnológicas del futuro](03-guias-de-carrera/las-50-carreras-tecnologicas-del-futuro.md)
- [50 guías individuales de carrera](03-guias-de-carrera/README.md)
- Recursos educativos gratuitos y herramientas de código abierto dentro de cada guía
- Proyectos de portafolio y orientación para GitHub
- Planes de aprendizaje de 30, 60 y 90 días
- Accesibilidad, adaptaciones y seguridad humana

Las guías de oportunidades, los recursos adicionales de inglés y otros materiales continuarán ampliándose progresivamente.

---

## Autor

**Alberto (Al) Leiva**

Este proyecto refleja un compromiso con la tecnología, la educación, la accesibilidad, la rehabilitación, la ciberseguridad, la mentoría y la creación de oportunidades para personas que pueden no haber tenido acceso a un camino claro.

---

## Licencia

Este proyecto utiliza la licencia **Creative Commons Atribución-NoComercial-CompartirIgual 4.0 Internacional**, conocida como **CC BY-NC-SA 4.0**.

Esto permite compartir y adaptar el contenido para fines no comerciales, siempre que se reconozca la autoría y las obras derivadas se distribuyan bajo la misma licencia.

Copyright © 2026 Alberto (Al) Leiva
